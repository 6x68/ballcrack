...
export default class ArrayList extends Module {
	...
	getAccentColors() {
		...
		return ["--Ballcrack-accent-color-1", "--Ballcrack-accent-color-2"].map(
			(v) => styles.getPropertyValue(v).trim(),
		);
	}
	...
	update(name, enabled) {
		if (enabled) {
			if (!this.namesMap[name]) {
				...
				const accentColors = this.getAccentColors();
				const bgOpacity = parseFloat(this.options["Background Opacity"]);
				const darknessMultiplier = parseFloat(
					this.options["Darkness Multiplier"],
				);
				const accentDarkness = parseFloat(this.options["Accent Darkness"]);
				const blur = parseFloat(this.options.Blur);
				...
				moduleElement.style.transition =
					"opacity 0.2s ease-in-out, max-height 0.2s ease-in-out";
				...
				textElem.style.backgroundImage = "var(--Ballcrack-accent-color)";
	...
	onEnable() {
		if (!this.initialized) {
			...
			this.arraylistContainer.style.zIndex = "99999";
			...
			this.arraylistContainer.style.borderImage =
				"var(--Ballcrack-accent-color)";
