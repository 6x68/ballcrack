...
export default class ClickGUI extends Module {
	...
	updateColors() {
		...
		shadowWrapper.wrapper.style.setProperty(
			"--Ballcrack-accent-color",
			accentGradient,
		);
		shadowWrapper.wrapper.style.setProperty(
			"--Ballcrack-accent-color",
		...
		shadowWrapper.wrapper.style.setProperty(
			"--Ballcrack-accent-color-1",
			this.options["Accent Color 1"],
		);
		shadowWrapper.wrapper.style.setProperty(
			"--Ballcrack-accent-color-2",
			this.options["Accent Color 2"],
		);
		shadowWrapper.wrapper.style.setProperty(
			"--Ballcrack-button-color",
		...
		shadowWrapper.wrapper.style.setProperty(
			"--hover-color",
			this.options["Hover Color"],
		);
		shadowWrapper.wrapper.style.setProperty(
			"--header-bg",
			this.options["Header Color"],
		);
		shadowWrapper.wrapper.style.setProperty(
			"--panel-bg",
			this.options["Panel Color"],
		);
		shadowWrapper.wrapper.style.setProperty(
			"--text-color",
		...
		shadowWrapper.wrapper.style.setProperty(
			...
			colorUtils.hexToRGBA(
				this.options["Accent Color 1"],
				parseFloat(this.options["Glow Alpha"]),
				1.2,
			),
		);
	}
	...
	setupEventListeners() {
		events.on("module.update", (module) => {
			const panel = this.panels.find(
				(p) => p.header.textContent === module.category,
			);
			if (!panel) return;

			const button = panel.buttons.find(
				(btn) => btn.textContent === module.name,
			);
