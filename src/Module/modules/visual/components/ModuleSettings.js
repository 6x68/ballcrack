...
export default class ModuleSettings {
	...
	closeAllDropdowns() {
		...
		if (this.activeDropdownListeners) {
			document.removeEventListener(
				"click",
				this.activeDropdownListeners.outsideClickHandler,
			);
			window.removeEventListener(
				"scroll",
				this.activeDropdownListeners.hideDropdown,
				true,
			);
			window.removeEventListener(
				"resize",
	...
	addNumberInput(name) {
		...
		input.addEventListener("blur", () => {
			if (Number.isNaN(input.value) || input.value.trim() === "") {
				input.value = this.module.options[name];
			}
		});
	...
	addModeSelector(name) {
		...
		const performHideDropdownActions = () => {
			...
			if (
				...
				this.activeDropdownListeners.dropdown === dropdown
			) {
				document.removeEventListener(
					"click",
					this.activeDropdownListeners.outsideClickHandler,
		...
		const specificOutsideClickHandler = (e) => {
			...
			if (
				...
				(!optionsList || !optionsList.contains(e.target))
			) {
			...
			setTimeout(() => {
				if (
					...
					window.addEventListener(
						"scroll",
						specificScrollResizeHideHandler,
						true,
					);
					window.addEventListener(
						"resize",
	...
	positionDropdown(dropdown, optionsList) {
		...
		if (optionsList.getBoundingClientRect().right > containerRect.right) {
			const overflow =
				optionsList.getBoundingClientRect().right - containerRect.right;
